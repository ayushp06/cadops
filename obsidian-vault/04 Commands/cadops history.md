...
- Reads `.cadops/previews/manifest.json` from each commit when available and uses it for preview availability annotations.
...
- Falls back cleanly to the standard CAD file list with unavailable annotations when commit-scoped metadata or preview records are absent.
