...
- Regenerates the lightweight `.cadops/previews/manifest.json` before commit so preview records stay near CAD snapshots.
- Includes metadata and preview manifests in the snapshot commit when refresh succeeds and warns without blocking when either refresh fails.
